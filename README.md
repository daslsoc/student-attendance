...
- **In-app help guide.** A screenshot-led *Help* page (in the nav, behind
  login) walks teachers through every task in plain language — logging in,
  marking attendance, book distribution, back-filling, and the reports. The
  screenshots are committed under `public/images/help/` and regenerated by a
  guarded Dusk capture (see Testing below).
- **Roles & permissions.** Every teacher holds exactly one role
  (Administrator / Coordinator / Teacher are seeded), and every page is gated
  by a permission atom from `config/permissions.php` — enforced server-side on
  each route, with the nav only showing what the role allows. Administrators
  edit roles through a checkbox grid (*Admin → Roles & Permissions*); changes
  apply to members immediately, no re-login needed.
- **Teacher administration.** *Admin → Teachers* adds an account with just a
  name, email and role — no password, login stays magic-link only. Removing
  someone **deactivates** rather than deletes: no more login links, links
  already in their inbox stop working, and a live session is dropped on its
  next request, while the row survives for the audit trail. Guards stop you
  deactivating yourself or removing the last account able to manage teachers.
- **Audit log.** Every account and permission change is recorded in an
  append-only activity log — who did it, when, and before/after detail —
  readable at *Admin → Audit Log*, with no edit or delete path in the app.

Only people in the `users` table (added via *Admin → Teachers*, or seeded) can
request a login link. Students appear on a form only if they are enrolled in
that `(subject, class)`.
...
CI (`.github/workflows/tests.yml`) runs PHPUnit, Pint, and Vitest on every
push / PR to `main`.

The teacher **Help page** screenshots are produced by a guarded Dusk capture
(`tests/Browser/HelpScreenshotsCapture.php`) — a documentation tool, not a
regression test, so it is skipped on a normal run. To refresh the images:
...
where `resources/views/help/index.blade.php` references it — keep the filenames
in sync if you rename one.
